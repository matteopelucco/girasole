# Archivio di TASKS.md

Storico completo delle voci fino al 2026-09-25, spostato da `TASKS.md`
per non caricarlo in ogni sessione. Le voci ancora aperte sono riportate
in `TASKS.md`.
...
## Costi API: review e triage più economici, contesto ridotto (2026-09-25)
- [x] `claude-board.yml`: triage con `Read,Grep,Glob,Write` (i tentativi
      negati consumavano tutti i turni); review solo su PR non in draft e
      senza etichetta `skip-review`, diff preparato in `.review/diff.txt`
      (senza `TASKS.md`, archivio, lockfile), `Write` e `git diff/log`
      permessi, `--max-turns` 30 → 15. Misurato prima: review #76 $0,99
      (fallita), review #74 $0,38, triage #77 $0,10 (fallito).
- [x] `TASKS.md` ridotto alle sole voci aperte (209 KB → 14 KB); storico in
